...
import { bisectionMethod } from '../../api/client';
...
export function BisectionInputs() {
  ...
  const handleCalculate = async () => {
    ...
    try {
      ...
      if (maxIter) payload.max_iter = parseInt(maxIter, 10);

      const res = await bisectionMethod(payload);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <div className="space-y-4">
        ...
        <MathInput instanceKey="bisection_func" onLatexChange={setFuncLatex} initialLatex={funcLatex} />
              ...
              <div>
                    ...
                    <tbody>
                      {(result.table || []).map((row: any) => (
                        <tr key={row.iteration} className="odd:bg-white/3 even:bg-white/5 text-blue-200">
                          ...
                          <td className="p-2">{fmt(row.interval_length)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="mt-3 flex gap-3">
                  <Dialog>
                    ...
                    <DialogContent className="backdrop-blur-xl bg-white/5 border border-white/10 rounded-2xl p-6 w-[95vw] max-w-3xl max-h-[70vh] mx-auto overflow-hidden">
                      <DialogTitle><span className='text-gray-200'>Iteraciones estimadas</span></DialogTitle>
                      <DialogDescription>
                        <div className="mt-2 text-gray-400 overflow-auto max-h-[56vh] pr-2">
                          <BlockMath>{result.iterations_estimate?.latex?.formula_general}</BlockMath>
                          <BlockMath>{result.iterations_estimate?.latex?.formula_substitution}</BlockMath>
                          <BlockMath>{result.iterations_estimate?.latex?.formula_numeric}</BlockMath>
                  ...
                  <Dialog>
                    ...
                    <DialogContent className="backdrop-blur-xl bg-white/5 border border-white/10 rounded-2xl p-6
                                                w-[45vw] max-w-[45vw] sm:max-w-[45vw]
                                                max-h-[80vh] mx-auto overflow-hidden">
                      <DialogTitle><span className='text-gray-200'>Conclusiones</span></DialogTitle>
                      <DialogDescription>
                        <div className="mt-2 text-gray-400 overflow-auto max-h-[46vh] pr-2">
                          <BlockMath>{result.conclusion?.latex}</BlockMath>
