...
import { secantMethod } from '../../api/client';
...
export function SecantInputs() {
  ...
  const [angleMode, setAngleMode] = React.useState<string>('rad');
  ...
  const handleCalculate = async () => {
    ...
    try {
      ...
      const res = await secantMethod(payload);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <div className="space-y-4">
        ...
        <MathInput instanceKey="secant_func" onLatexChange={setFuncLatex} initialLatex={funcLatex} />

        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-sm text-blue-200">x<sub>-1</sub></label> 
          ...
          <div>
            <label className="text-sm text-blue-200">x<sub>0</sub></label>
            <input className="w-full p-2 rounded bg-white/5 border border-white/10 text-blue-200" value={x1} onChange={(e) => setX1(e.target.value)} placeholder="1.0" />
          </div>
        </div>
        ...
        <div>
          <label className="text-sm text-blue-200">Angle Mode</label>
          <select value={angleMode} onChange={(e) => setAngleMode(e.target.value)} className="w-full p-2 rounded bg-white/5 border border-white/10 text-blue-200">
                  ...
                  <table className="w-full text-sm table-auto border-collapse">
                    <thead>
                      <tr className="text-left text-xs text-gray-300">
                        ...
                        <th className="p-2">x<sub>0</sub></th>
                        <th className="p-2">x<sub>i+1</sub></th>
                        <th className="p-2">f(x<sub>i-1</sub>)</th>
                    ...
                    <tbody>
                      {(result.table || []).map((row: any) => (
                        <tr key={row.iteration} className="odd:bg-white/3 even:bg-white/5 text-blue-200">
                          ...
                          <td className="p-2">{fmt(row.x_prev)}</td>
                          <td className="p-2">{fmt(row.x_curr)}</td>
                          <td className="p-2">{fmt(row.x_next)}</td>
                          <td className="p-2">{fmt(row.f_prev)}</td>
                          <td className="p-2">{fmt(row.f_curr)}</td>
                          <td className="p-2">{fmt(row.f_next)}</td>
                    ...
                    <DialogContent className="backdrop-blur-xl bg-white/5 border border-white/10 rounded-2xl p-6 w-[min(1200px,96vw)] max-w-6xl max-h-[90vh] mx-auto">
                      ...
                      <DialogDescription>
                        <div className="mt-2 text-gray-400 pr-2 w-full">
                          <BlockMath className="w-full">{result.iterations_estimate?.latex?.formula_secant}</BlockMath>
